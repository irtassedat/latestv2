// src/components/EnhancedBranchManager.jsx
import { useState, useEffect } from "react";
import { FiEdit2, FiTrash2, FiPlus, FiMapPin, FiPhone, FiMail, FiHome, FiInfo, FiSettings, FiPackage, FiLayers, FiShoppingBag, FiTruck, FiShoppingCart } from "react-icons/fi";
...
const EnhancedBranchManager = () => {
    ...
    // Marka, şubeler ve şablonları getir
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingBranch) {
                ...
                await api.post(`/api/branches/${editingBranch.id}/integrations`, {
                ...
                if (response.data && response.data.id) {
                    await api.post(`/api/branches/${response.data.id}/integrations`, {
                        integration_ids: selectedIntegrations
                    });
                }
    ...
    const handleTemplateSubmit = async (e) => {
        ...
        try {
            // Şablonları güncelle
            await api.patch(`/api/branches/${editingBranch.id}/templates`, {
    ...
    const handleGenerateQR = (branch) => {
        ...
        const qrUrl = `${window.location.origin}/menu/${branch.id}`;

        // Yeni bir sekme açarak QR kodu göster
        const newWindow = window.open('', '_blank');
        newWindow.document.write(`
      <html>
        <head>
          <title>${branch.name} QR Code</title>
        ...
        <body style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; margin: 0; font-family: Arial, sans-serif;">
          <h2 style="margin-bottom: 20px; color: #022B45;">${branch.name} - QR Kod</h2>
          <div id="qrcode"></div>
          <p style="margin-top: 20px;">${qrUrl}</p>
          <button onclick="window.print()" style="margin-top: 20px; padding: 10px 20px; background-color: #022B45; color: white; border: none; border-radius: 4px; cursor: pointer;">QR Kodu Yazdır</button>
          <script>
            var typeNumber = 4;
